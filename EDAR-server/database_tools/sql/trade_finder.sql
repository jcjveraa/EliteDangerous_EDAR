select
	table1.commodity_id,
	table1.station_id as buy_station_id,
	buy_station_full.name as buy_station_name,
	table2.station_id as sell_station_id,
	(buy_system_full.X - sell_system_full.X) *(buy_system_full.X - sell_system_full.X) + (buy_system_full.Y - sell_system_full.Y) *(buy_system_full.Y - sell_system_full.Y) + (buy_system_full.Z - sell_system_full.Z) *(buy_system_full.Z - sell_system_full.Z) as jump_distance_LY_Squared,
	...
	buy_station_full.system_id as buy_system_id,
	buy_system_full.name as buy_system_name,
	sell_station_full.system_id as sell_system_id,
	sell_system_full.name as sell_system_name
from
	listings_v6 as table1
	INNER join (
		select
			*,
			max(sell_price)
		...
		where
			listings_v6.station_id in (
				select station_id from @@@TEMP_TABLE_NAME@@@ where max_landing_pad_size >= @pad_sizes
			)
			and demand > 0
...
where
	table1.station_id in (
		...
		from
			stations_v6
		where
			stations_v6.system_id = @system_id
			and stations_v6.max_landing_pad_size >= @pad_sizes
	)
	and table1.supply != 0
